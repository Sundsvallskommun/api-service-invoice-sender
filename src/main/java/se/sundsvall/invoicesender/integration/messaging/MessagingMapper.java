...
public class MessagingMapper {
	...
	public DigitalInvoiceRequest toDigitalInvoiceRequest(final ItemEntity invoice, final String path) throws IOException {
		try (var in = new FileInputStream(fileSystem.getPath(path).resolve(invoice.getFilename()).toFile());
			final var out = new ByteArrayOutputStream()) {
			IOUtils.copyLarge(in, out);

			final var encodedInvoiceContent = new String(Base64.getEncoder().encode(out.toByteArray()), UTF_8);
